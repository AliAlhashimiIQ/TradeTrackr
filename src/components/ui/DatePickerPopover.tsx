    ...
    <div className="relative inline-block" ref={popoverRef}>
      ...
      <div className="flex items-center gap-1.5 bg-white dark:bg-[var(--surface-1)] border border-slate-200/80 dark:border-white/[0.08] p-1.5 rounded-2xl shadow-sm">
        ...
        <button
          ...
          className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl transition-all font-semibold text-xs active:scale-95"
          title="Previous Day"
        ...
        >
          ...
          <span className="font-sans font-semibold tracking-tight">{formattedDisplay}</span>
        </button>
        ...
        <button
          ...
          className="px-3.5 py-1.5 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 hover:bg-indigo-500/20 rounded-xl transition-all font-extrabold text-xs active:scale-95"
        >
          Today
        </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-2 z-50 w-72 bg-white/95 dark:bg-[#0c0e18] border border-slate-200/90 dark:border-white/[0.12] rounded-3xl p-4 shadow-2xl shadow-black/60 backdrop-blur-2xl text-slate-900 dark:text-slate-100"
          >
            ...
            <div className="grid grid-cols-7 gap-1 text-center mb-2">
              {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((day) => (
                <span key={day} className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase font-mono">
                  {day}
                </span>
              ))}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`h-8 w-8 rounded-xl text-xs font-bold font-mono transition-all flex items-center justify-center ${
                      isSelected
                        ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/40 scale-105 ring-1 ring-indigo-400'
                        : isToday
                        ? 'border border-indigo-500/60 text-indigo-400 bg-indigo-500/10'
                        : 'hover:bg-white/[0.08] hover:text-white text-slate-700 dark:text-slate-300'
                    }`}
                  >
                    {dayNum}
                  </button>
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 dark:border-white/[0.06] flex items-center justify-between text-xs">
              <button
                type="button"
                onClick={handleSelectToday}
                className="px-2.5 py-1 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 text-[11px] font-bold transition-colors font-mono"
              >
                Jump to Today
              </button>
              <button
                ...
                className="px-2.5 py-1 rounded-lg hover:bg-white/[0.06] text-slate-400 hover:text-white text-[11px] font-medium transition-colors"
              >
                Close
              </button>
            </div>
