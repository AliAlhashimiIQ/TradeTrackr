        ...
        {isOpen && (
          ...
          >
            ...
            <div className="px-4 py-2.5 border-b border-slate-100 dark:border-white/[0.06] flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Trading Account</p>
              {!isAll && (
                <button
                  onClick={() => setSelection('all')}
                  className="text-[10px] text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 font-semibold transition-colors"
                >
                  Clear
                </button>
              )}
            ...
            {/* List */}
            <div className="max-h-60 overflow-y-auto divide-y divide-slate-100 dark:divide-white/[0.04] scrollbar-thin">
              {/* All Accounts Option */}
              <button
                onClick={() => {
                  setSelection('all')
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-100">All Accounts</span>
                </div>
              </button>

              {/* Individual accounts — checkboxes for multi-select */}
              {accounts.length === 0 ? (
                <div className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 italic text-center">
                ...
                accounts.map((acc) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Name */}
                      <span className="truncate flex-1 font-semibold text-slate-800 dark:text-slate-100" title={acc.name}>
                        {acc.name}
                      </span>
            ...
            <div className={`${accounts.length > 1 ? '' : 'border-t border-slate-100 dark:border-white/[0.04] mt-1.5'} pt-1.5 px-2 pb-1`}>
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 py-2 bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-300 rounded-xl text-xs font-bold border border-indigo-200 dark:border-indigo-800/40 transition-all"
              >
