...
const BADGES: BadgeDef[] = [
  {
    ...
    accentColor: '#f59e0b', glowColor: 'rgba(245,158,11,0.08)',
  ...
  },
  {
    ...
    accentColor: '#eab308', glowColor: 'rgba(234,179,8,0.08)',
  ...
  },
  {
    ...
    accentColor: '#10b981', glowColor: 'rgba(16,185,129,0.08)',
  ...
  },
  {
    ...
    accentColor: '#6366f1', glowColor: 'rgba(99,102,241,0.08)',
  ...
  },
  {
    ...
    accentColor: '#8b5cf6', glowColor: 'rgba(139,92,246,0.08)',
  ...
  },
  {
    ...
    accentColor: '#06b6d4', glowColor: 'rgba(6,182,212,0.08)',
  ...
  },
  {
    ...
    accentColor: '#14b8a6', glowColor: 'rgba(20,184,166,0.08)',
  ...
  },
  {
    ...
    accentColor: '#f43f5e', glowColor: 'rgba(244,63,94,0.08)',
  ...
  return (
    <div className="relative flex items-center justify-center shrink-0" style={{ width: 120, height: 120 }}>
      ...
      <svg width="120" height="120" viewBox="0 0 120 120" className="absolute inset-0 -rotate-90">
        ...
        <circle cx="60" cy="60" r={r} fill="none" stroke="currentColor" className="text-black/[0.04] dark:text-white/[0.03]" strokeWidth="8" />
...
function StatCard({ label, value, accent }: { label: string; value: number | string; accent?: string }) {
  return (
    <div className="flex flex-col items-center gap-1.5 py-3 rounded-xl border border-slate-200/60 dark:border-white/[0.05] bg-slate-50/50 dark:bg-white/[0.015] transition-colors">
      <span
  ...
  return (
    <motion.div
      className={`relative flex items-center gap-3 px-3.5 py-3 rounded-xl border transition-all duration-300 cursor-default group/badge overflow-hidden ${
        earned
          ? 'border-indigo-500/10 dark:border-indigo-500/20 shadow-sm'
          : 'bg-slate-50 dark:bg-white/[0.015] border-slate-200/50 dark:border-white/[0.04] opacity-50'
      }`}
      style={earned ? {
    ...
    >
      ...
      <div
        className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 relative ${
          earned ? '' : 'text-slate-400 dark:text-white/20 bg-slate-100 dark:bg-white/[0.03]'
        }`}
        style={earned ? {
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-[11px] font-semibold leading-tight ${earned ? '' : 'text-slate-500 dark:text-white/25'}`}
          style={earned ? { color: badge.accentColor } : undefined}>
          {badge.name}
        </p>
        <p className={`text-[9px] mt-0.5 leading-tight ${earned ? 'text-slate-600 dark:text-white/40' : 'text-slate-400 dark:text-white/12'}`}>
          {badge.description}
        </p>
      </div>
      ...
      {earned ? (
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4 text-slate-300 dark:text-gray-700 shrink-0">
  ...
  return (
    <div className="flex flex-col gap-1.5">
                ...
                <div
                  ...
                  className={`h-[12px] rounded-[3px] transition-all duration-200 ${
                    cell.isFuture || cell.isWeekend
                      ? 'bg-transparent'
                      : cell.hasActivity
                      ? 'bg-indigo-500/80 dark:bg-indigo-500/80'
                      : cell.isFrozen
      ...
      <div className="flex items-center justify-end gap-4 mt-0.5">
        {[
          { className: 'bg-indigo-500/80', label: 'Active' },
          { className: 'bg-cyan-500/50', label: 'Freeze' },
          { className: 'bg-slate-100 dark:bg-white/[0.04]', label: 'Inactive' },
        ].map(item => (
...
function FreezeTokens({ count }: { count: number }) {
  return (
    <div className="flex items-center gap-3 p-4 rounded-xl border border-cyan-500/10 dark:border-cyan-500/20 bg-cyan-500/[0.02] dark:bg-cyan-500/[0.03]">
          ...
          return (
            <motion.div
              ...
              className={`relative w-10 h-10 rounded-xl border flex items-center justify-center ${
                active
                  ? 'bg-cyan-500/10 dark:bg-cyan-500/20 border-cyan-500/30 text-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.15)]'
                  : 'bg-slate-100/70 dark:bg-white/[0.02] border-slate-200 dark:border-white/[0.05] text-slate-300 dark:text-white/10'
              }`}
              initial={false}
      ...
      <div className="text-right">
        <p className="text-sm font-bold tabular-nums text-cyan-600 dark:text-cyan-400">
          {count}<span className="text-slate-400 dark:text-slate-600 font-normal"> / 3</span>
        </p>
        <p className="text-[9px] text-gray-500 dark:text-gray-600 mt-0.5 font-medium">available</p>
      </div>
    </div>
  )
}
...
function SectionHeader({ title, right }: { title: string; right?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between mb-3">
      <h3 className="text-[10px] font-semibold uppercase tracking-[0.14em] text-gray-500 dark:text-gray-400">{title}</h3>
      {right}
    </div>
  )
}
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div
              className="relative w-full max-w-[520px] max-h-[90vh] overflow-y-auto overflow-x-hidden rounded-2xl pointer-events-auto border border-slate-200 dark:border-white/[0.06] shadow-2xl"
              style={{
                background: 'linear-gradient(180deg, var(--card-bg) 0%, var(--card-bg) 100%)',
                boxShadow: '0 40px 100px rgba(0,0,0,0.2), 0 0 0 1px rgba(255,255,255,0.03) inset, 0 0 60px rgba(99,102,241,0.02)',
              }}
            >
              {/* ─── Header ─── */}
              <div className="sticky top-0 z-10 flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100 dark:border-white/[0.04]"
                style={{ background: 'var(--card-bg)' }}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-500 dark:text-indigo-400">
                ...
                <button
                  onClick={onClose}
                  className="p-2 -mr-1 rounded-lg text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                  aria-label="Close"
                >
              ...
              <div className="px-6 pb-6 flex flex-col gap-5 pt-5">
                ...
                {/* ─── Hero: Ring + Status ─── */}
                <div className="flex items-center gap-5 p-5 rounded-xl border border-indigo-500/10 dark:border-indigo-500/20 relative overflow-hidden bg-gradient-to-br from-indigo-500/[0.04] to-violet-500/[0.02] dark:from-indigo-500/[0.06] dark:to-violet-500/[0.03]">
                  ...
                  <div className="flex-1 min-w-0 relative">
                    ...
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-semibold uppercase tracking-[0.12em] mb-3"
                      ...
                      <div className="relative">
                        <div className={`w-1.5 h-1.5 rounded-full ${isStreakActiveToday ? 'bg-emerald-500' : 'bg-red-500'}`} />
                        {isStreakActiveToday && (
                    ...
                    <div className="space-y-2">
                      ...
                      {/* Progress bar */}
                      <div className="relative w-full h-2 rounded-full overflow-hidden bg-slate-100 dark:bg-white/5">
                        <motion.div
                      ...
                      <div className="flex items-center justify-between text-[10px]">
                        ...
                        <span className="text-gray-500">
                          Best: <span className="text-gray-700 dark:text-gray-300 font-semibold tabular-nums">{longestStreak}d</span>
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <StatCard label="Week" value={thisWeek} accent={thisWeek >= 3 ? '#10b981' : undefined} />
                ...
                <div>
                  ...
                  <div className="p-4 rounded-xl border border-slate-200/60 dark:border-white/[0.04]"
                    style={{ background: 'var(--card-bg)' }}>
